...
export default function AuthPortal({ authMode, setAuthMode, authRole, setAuthRole, setCurrentUser, setCurrentPage, triggerToast, onLogin, onRegister }) {
  // Form Input States
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-16 relative z-10">
      <div className="p-8 rounded-3xl backdrop-blur-md bg-white/70 dark:bg-slate-900/80 border border-slate-200/50 dark:border-slate-800/50 shadow-2xl space-y-6">
        
        {/* Header tab to switch roles (Vendor / HR) */}
        {authMode !== 'otp' && (
          <div className="flex p-1 rounded-xl bg-slate-100 dark:bg-slate-950 border border-slate-200/50 dark:border-slate-800/50">
            <button 
              onClick={() => { setAuthRole('vendor'); setAuthMode('login'); }}
              className={`flex-grow py-2 text-xs font-bold uppercase rounded-lg tracking-wider transition-all ${authRole === 'vendor' ? 'bg-white dark:bg-slate-900 text-tata-600 dark:text-tata-400 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200'}`}
            >
              Vendor Portal
            </button>
            <button 
        ...
        <div className="text-center space-y-1">
          <h2 className="font-heading font-extrabold text-2xl text-slate-900 dark:text-white uppercase tracking-tight">
          ...
          <form onSubmit={handleLoginSubmit} className="space-y-4">
            ...
            <div className="space-y-1">
              <div className="flex justify-between items-center">
                ...
                <button type="button" onClick={() => setAuthMode('forgot')} className="text-[10px] font-semibold text-tata-500 hover:underline">Forgot Password?</button>
              </div>
              <input 
                type="password" 
                required 
            ...
            <button 
              type="submit" 
              className="w-full py-3.5 bg-gradient-to-r from-tata-600 to-tata-500 hover:from-tata-700 hover:to-tata-600 text-white font-bold text-xs uppercase tracking-wider rounded-xl transition-all shadow-md shadow-tata-500/10"
            >
              Sign In
            </button>
            ...
            {authRole === 'vendor' && (
              <p className="text-center text-xs text-slate-500">
                New Vendor?{' '}
                <button type="button" onClick={() => setAuthMode('signup')} className="font-bold text-tata-500 hover:underline">Register Business</button>
              </p>
            )}
          </form>
        )}
        ...
        {authMode === 'signup' && (
          <form onSubmit={handleSignupSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                <label className="block text-xs font-semibold text-slate-600 dark:text-slate-400">GST Number</label>
                <input 
            ...
            <button 
              type="submit" 
              className="w-full py-3 bg-gradient-to-r from-tata-600 to-tata-500 hover:from-tata-700 hover:to-tata-600 text-white font-bold text-xs uppercase tracking-wider rounded-xl transition-all shadow-md"
            >
              Verify GST & Proceed
            </button>
            ...
            <p className="text-center text-xs text-slate-500">
              Already registered?{' '}
              <button type="button" onClick={() => setAuthMode('login')} className="font-bold text-tata-500 hover:underline">Sign In</button>
            </p>
          </form>
        )}
        ...
        {authMode === 'forgot' && (
          <div className="space-y-4">
            ...
            <button 
              ...
              className="w-full py-3 bg-gradient-to-r from-tata-600 to-tata-500 text-white font-bold text-xs uppercase tracking-wider rounded-xl transition-all"
            >
              Send Reset Link
            </button>
            <div className="text-center">
              <button onClick={() => setAuthMode('login')} className="text-xs font-bold text-slate-500 hover:underline">Back to Login</button>
            </div>
          </div>
        )}
        ...
        {authMode === 'otp' && (
          <form onSubmit={handleOtpSubmit} className="space-y-6">
            <div className="text-center space-y-2">
              ...
              <p className="text-xs font-semibold text-tata-500">(Enter 1234 or any 4 digits to bypass)</p>
            </div>
            ...
            <div className="flex justify-center gap-4">
              {otpInput.map((val, idx) => (
                <input 
                  ...
                  className="w-12 h-14 text-center text-xl font-bold rounded-xl border-2 border-slate-200 dark:border-slate-800 bg-white/50 dark:bg-slate-950/50 focus:outline-none focus:border-tata-500 focus:ring-2 focus:ring-tata-500/25 transition-all"
                />
              ))}
            </div>
            ...
            <button 
              type="submit" 
              className="w-full py-3.5 bg-gradient-to-r from-tata-600 to-tata-500 text-white font-bold text-xs uppercase tracking-wider rounded-xl transition-all"
            >
              Submit & Authenticate
            </button>
            ...
            <div className="flex justify-between items-center text-xs">
              ...
              <button type="button" onClick={() => triggerToast('OTP dispatched again.')} className="font-bold text-tata-500 hover:underline">Resend OTP</button>
            </div>
          </form>
        )}
