...
export default function LandingPage({ setCurrentPage, setAuthRole, setAuthMode, tenders }) {
  const liveBidsCount = tenders.filter(t => t.status === 'Open').length;
  
  return (
    <div className="w-full">
      {/* Hero Section */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 lg:py-28 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center relative">
        <div className="lg:col-span-7 space-y-6 text-center lg:text-left">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-tata-500/10 text-tata-600 dark:bg-tata-500/20 dark:text-tata-400 border border-tata-500/10 dark:border-tata-500/20 text-xs font-semibold uppercase tracking-wider">
          ...
          <div className="flex flex-wrap items-center justify-center lg:justify-start gap-4 pt-4">
            ...
            <button 
              ...
              className="px-8 py-3.5 border border-slate-200 dark:border-slate-800 bg-white/50 dark:bg-slate-900/50 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-800 dark:text-slate-200 font-bold rounded-xl shadow-sm transition-all text-sm uppercase tracking-wide"
            >
              Vendor Login
            </button>
            <button 
        ...
        <div className="lg:col-span-5 relative flex justify-center">
          <div className="w-full max-w-[28rem] aspect-square relative rounded-3xl p-1 bg-gradient-to-tr from-tata-500/20 via-slate-200 to-tata-500/10 dark:from-slate-800 dark:via-slate-900 dark:to-slate-950 shadow-2xl flex items-center justify-center overflow-hidden border border-white/20">
            <div className="absolute inset-0 bg-gradient-to-br from-slate-900/5 to-slate-900/20 dark:from-slate-950/20 dark:to-slate-950/60 z-0"></div>
            
            {/* SVG Gears Animation */}
            <div className="absolute top-[10%] left-[10%] opacity-20 dark:opacity-10 text-tata-500">
              <svg className="w-32 h-32 animate-gear-slow" viewBox="0 0 100 100" fill="currentColor">
            ...
            <div className="absolute bottom-[10%] right-[10%] opacity-20 dark:opacity-10 text-tata-300">
              <svg className="w-40 h-40 animate-gear-slow-reverse" viewBox="0 0 100 100" fill="currentColor">
                <path d="M50 34c-8.8 0-16 7.2-16 16s7.2 16 16 16 16-7.2 16-16-7.2-16-16-16zm0 24c-4.4 0-8-3.6-8-8s3.6-8 8-8 8 3.6 8 8-3.6 8-8 8zm0-38c-1.7 0-3 1.3-3 3v4.2c-2.4.5-4.7 1.5-6.8 2.8l-3-3c-1.2-1.2-3.1-1.2-4.2 0l-4.2 4.2c-1.2 1.2-1.2 3.1 0 4.2l3 3c-1.3 2.1-2.3 4.4-2.8 6.8H25c-1.7 0-3 1.3-3 3v6c0 1.7 1.3 3 3 3h4.2c.5 2.4 1.5 4.7 2.8 6.8l-3 3c-1.2 1.2-1.2 3.1 0 4.2l4.2 4.2c1.2 1.2 3.1 1.2 4.2 0l3-3c2.1 1.3 4.4 2.3 6.8 2.8V75c0 1.7 1.3 3 3 3h6c1.7 0 3-1.3 3-3v-4.2c2.4-.5 4.7-1.5 6.8-2.8l3 3c1.2 1.2 3.1 1.2 4.2 0l4.2-4.2c1.2-1.2 1.2-3.1 0-4.2l-3-3c1.3-2.1 2.3-4.4 2.8-6.8H75c1.7 0 3-1.3 3-3v-6c0-1.7-1.3-3-3-3h-4.2c-.5-2.4-1.5-4.7-2.8-6.8l3-3c1.2-1.2 1.2-3.1 0-4.2l-4.2-4.2c-1.2-1.2-3.1-1.2-4.2 0l-3 3c-2.1-1.3-4.4-2.3-6.8-2.8V25c0-1.7-1.3-3-3-3h-6z"/>
              </svg>
            </div>

            {/* Floating Glass Dashboard Panel */}
            <div className="relative z-10 w-[90%] p-6 rounded-2xl backdrop-blur-md bg-white/70 dark:bg-slate-900/80 border border-white/20 dark:border-slate-800/40 shadow-xl space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-tata-600 dark:text-tata-400">Live Auction Stream</span>
                <span className="flex items-center gap-1.5 text-[10px] bg-red-500/10 text-red-500 px-2 py-0.5 rounded-full font-bold">
              ...
              <div className="space-y-3">
                <div className="p-3 bg-white/40 dark:bg-slate-950/40 rounded-xl flex items-center justify-between border border-white/10">
                  ...
                  <div className="text-right">
                    ...
                    <span className="text-[9px] text-slate-400">{tenders[0]?.lowestBidder}</span>
                  </div>
                </div>
                <div className="p-3 bg-white/40 dark:bg-slate-950/40 rounded-xl flex items-center justify-between border border-white/10">
                  <div>
              ...
              <div className="h-20 flex items-end gap-2 pt-2 border-t border-slate-200/40 dark:border-slate-800/40">
                <div className="flex-grow bg-tata-500/20 dark:bg-tata-500/10 rounded-t h-[40%] animate-pulse"></div>
                <div className="flex-grow bg-tata-500/45 dark:bg-tata-500/25 rounded-t h-[75%]"></div>
                <div className="flex-grow bg-gradient-to-t from-tata-600 to-tata-400 rounded-t h-[95%]"></div>
                <div className="flex-grow bg-tata-500/30 dark:bg-tata-500/15 rounded-t h-[55%] animate-pulse"></div>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Stats Section */}
      <section className="bg-white/40 dark:bg-slate-900/30 border-y border-slate-200/50 dark:border-slate-800/50 py-10 relative z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 sm:grid-cols-3 gap-8 text-center">
          <div className="space-y-2">
            <span className="block font-heading font-extrabold text-4xl text-tata-600 dark:text-tata-400">142+</span>
            <span className="text-xs uppercase tracking-widest font-semibold text-slate-500 dark:text-slate-400">Active Verified Vendors</span>
          </div>
          <div className="space-y-2 border-y sm:border-y-0 sm:border-x border-slate-200/50 dark:border-slate-800/50 py-6 sm:py-0">
            <span className="block font-heading font-extrabold text-4xl text-tata-600 dark:text-tata-400">{liveBidsCount}</span>
            <span className="text-xs uppercase tracking-widest font-semibold text-slate-500 dark:text-slate-400">Live Bids Open Now</span>
          </div>
          <div className="space-y-2">
            <span className="block font-heading font-extrabold text-4xl text-tata-600 dark:text-tata-400">18,500 MT</span>
            <span className="text-xs uppercase tracking-widest font-semibold text-slate-500 dark:text-slate-400">Materials Procured (YTD)</span>
          </div>
        </div>
      </section>

      {/* Achievements / Features Section */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 relative z-10 space-y-12">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 relative z-10 border-t border-slate-200/50 dark:border-slate-800/50">
        <h3 className="font-heading font-bold text-center text-2xl text-slate-900 dark:text-white mb-10">Trusted by India's Top Material Vendors</h3>
